 ...
 * $Id: object_endian.c,v 1.2 2002/10/23 10:25:43 marek Exp $
 */
		...
		case MED_TYPE_UNSIGNED:
			...
			{	memrcpy(o->data+a->offset+a->length-n,buf,n);
				...
				{	if( s && ((char*)buf)[maxlen-1]&0x80 )
					...
					else	memset(o->data+a->offset,0,a->length-n);
				}
			}
			else
			{	memcpy(o->data+a->offset,buf,n);
				...
				{	if( s && ((char*)buf)[maxlen-1]&0x80 )
					...
					else	memset(o->data+a->offset+n,0,a->length-n);
				}
			}
#elif __BYTE_ORDER == __BIG_ENDIAN
			if( o->flags & OBJECT_FLAG_CHENDIAN )
...
{ struct medusa_attribute_s *a;
	...
	{	o->flags= (o->flags & ~OBJECT_FLAG_CHENDIAN)
			  | (flags&OBJECT_FLAG_CHENDIAN);
