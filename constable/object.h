...
struct class_handler_s {
	...
	int (*init_comm)(struct class_handler_s *ch, struct comm_s *c);
	int (*set_handler)(struct class_handler_s *ch, struct comm_s *c, struct object_s *o);
	int (*get_vs)(struct class_handler_s *ch, struct comm_s *c, struct object_s *o, vs_t *v, int i);
	struct tree_s *(*get_tree_node)(struct class_handler_s *ch, struct comm_s *c, struct object_s *o);
	struct space_s *(*get_primary_space)(struct class_handler_s *ch, struct comm_s *c, struct object_s *o);
	int (*enter_tree_node)(struct class_handler_s *ch, struct comm_s *c, struct object_s *o, struct tree_s *t);
};
...
struct class_s {
	...
	uintptr_t cinfo_mask;	/**< bitmap that determines which
				  * blocks are allocated in cinfo
				  */
	struct class_names_s *classname;
	struct comm_s *comm;
...
#define	PCINFO(object, ch, comm)	((uintptr_t *)((object)->data+(ch)->cinfo_offset[(comm)->conn])) // 64-bit or 32-bit size because a pointer will be stored here
...
int object_add_event(struct object_s *o, struct event_mask_s *e);
