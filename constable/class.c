...
struct class_names_s *get_class_by_name(char *name)
{
	...
	c = malloc(sizeof(struct class_names_s) + strlen(name) + 1);
	if (c == NULL)
		return NULL;
	c->classes = (struct class_s **)(comm_new_array(sizeof(struct class_s *)));
	if (c->classes == NULL) {
		free(c);
		return NULL;
	}
...
struct class_s *add_class(struct comm_s *comm, struct medusa_class_s *mc, struct medusa_attribute_s *a)
{
	...
	if (a == NULL)
		a = (struct medusa_attribute_s *)(mc + 1);
	...
	ci = get_attribute(c, "o_cinfo");
	if (ci != NULL) {
		c->cinfo_offset = ci->offset;
		c->cinfo_size = (ci->length)/sizeof(u_int32_t);
	} else
		c->cinfo_offset = c->cinfo_size = 0;
	ci = get_attribute(c, "name");
	if (ci != NULL) {
		c->name_offset = ci->offset;
		c->name_size = ci->length;
	} else
		c->name_offset = c->name_size = 0;
	ci = get_attribute(c, "med_oact");
	if (ci != NULL) {
		c->event_offset = ci->offset;
		c->event_size = ci->length;
	} else
		c->event_offset = c->event_size = 0;
	...
	ci = get_attribute(c, "med_sact");
	if (ci != NULL) {
		c->subject.event_offset = ci->offset;
		c->subject.event_size = ci->length;
	} else
		c->subject.event_offset = c->subject.event_size = 0;

	ci = get_attribute(c, "s_cinfo");
	if (ci != NULL) {
		c->subject.cinfo_offset = ci->offset;
		c->subject.cinfo_size = (ci->length)/sizeof(u_int32_t);
	} else
		c->subject.cinfo_offset = c->subject.cinfo_size = 0;

	c->subject.cinfo_mask = 0;

	p = get_class_by_name(c->m.name);
	if (p == NULL) {
		free(c);
		return NULL;
	}
...
int class_comm_init(struct comm_s *comm)
{
	...
	for (c = classes; c != NULL; c = c->next) {
		for (h = c->class_handler; h != NULL; h = h->next) {
			if (h->init_comm && h->init_comm(h, comm) < 0) {
				pthread_mutex_unlock(&classes_lock);
				return -1;
			}
		}
...
{
	int i;
	static const char * const t[] = {"END", "unsigned", "signed", "string", "bitmap"};
	char buf[32];
