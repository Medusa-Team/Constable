...
{
	// TODO: In case of multiple connections, this checks just the first one [0].
...
int object_cmp_vs(vs_t *vs, int n, struct object_s *o)
{
	...
	if (o->class->vs_attr[n] != NULL) {
		...
		while (i > 0) {
			i--;
			if (((vs_t *)(o->data + o->class->vs_attr[n]->offset))[i] & vs[i])
				return 1;
		}
		return 0;
	}
...
int object_add_vs(struct object_s *o, int n, vs_t *vs)
{
	...
	if (o->class->vs_attr[n] != NULL) {
		...
		while (i > 0) {
			i--;
			((vs_t *)(o->data + o->class->vs_attr[n]->offset))[i] |= vs[i];
		}
		return 0;
	}
