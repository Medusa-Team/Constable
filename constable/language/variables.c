...
struct object_s *alloc_var(char *name, struct medusa_attribute_s *attr, struct class_s *class)
{
	...
	if (class == NULL)
		...
		l = class->m.size;
	v = calloc(1, sizeof(struct object_s) + l);
	if (v == NULL)
		return NULL;
	v->next = NULL;
...
struct object_s *data_alias_var(char *name, struct object_s *o)
{
	...
	if (o == NULL)
		return NULL;
	v = malloc(sizeof(struct object_s));
	if (v == NULL)
		return NULL;
	v->next = NULL;
	v->flags = o->flags;
...
struct object_s *var_link(struct object_s **v, struct object_s *var)
{
	...
	for (; (*v) != NULL; v = &((*v)->next)) {
		r = strncmp((*v)->attr.name, var->attr.name, MEDUSA_ATTRNAME_MAX);
		if (r >= 0)
			break;
	}
	if (r == 0)
...
struct object_s *get_var(struct object_s *v, char *name)
{
	...
	for (; v != NULL; v = v->next) {
		r = strncmp(v->attr.name, name, MEDUSA_ATTRNAME_MAX);
		if (r >= 0) {
			if (r == 0)
				return v;
			else
