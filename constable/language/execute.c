...
struct medusa_attribute_s execute_attr_int = {
	0, 4, MED_TYPE_SIGNED | MED_TYPE_READ_ONLY, "",
};

struct medusa_attribute_s execute_attr_str = {
	0, MAX_REG_SIZE, MED_TYPE_STRING | MED_TYPE_READ_ONLY, "",
};

struct medusa_attribute_s execute_attr_pointer = {
	0, sizeof(void *), MED_TYPE_UNSIGNED | MED_TYPE_READ_ONLY, "",
};
...
void obj_to_reg(struct register_s *r, struct object_s *o, char *attr)
{
	...
	r->attr = get_attribute(o->class, attr);
	if (r->attr == NULL) {
...
#endif
	{
		o = get_var((struct object_s *)(execute_readstack(e, p)), name);
		if (o != NULL)
			return o;
	}

	o = get_var(e->c->local_vars, name);
	if (o != NULL)
		return o;
...
void reg_load_var(struct register_s *r, struct execute_s *e, char *name, char *attr)
{
	...
	if (attr == NULL) {
		if (e->c->operation.class != NULL) {
			r->attr = get_attribute(e->c->operation.class, name);
			if (r->attr != NULL) {
		...
		if (e->c->subject.class != NULL) {
			r->attr = get_attribute(e->c->subject.class, name);
			if (r->attr != NULL) {
		...
		if (e->c->object.class != NULL) {
			r->attr = get_attribute(e->c->object.class, name);
			if (r->attr != NULL) {
...
{
	return (char *)((struct register_s *)pthread_getspecific(r0_key))->data;
}

struct medusa_attribute_s *execute_get_last_attr(void)
{
	return ((struct register_s *)pthread_getspecific(r0_key))->attr;
}
...
static int execute_handler_do(struct execute_s *e)
{
	...
	for (;;) {
		...
		case oSTO:
			...
			break;
		case oFET:
			v = data_find_var(e, (char *)(*(e->p)++));
			if (v != NULL) {
				i = v->class->comm->fetch_object(v->class->comm, e->cont, v, e->my_comm_buff);
				if (i < 0) {
		...
		case oUPD:
			v = data_find_var(e, (char *)(*(e->p)++));
			if (v != NULL) {
				i = v->class->comm->update_object(v->class->comm, e->cont, v, e->my_comm_buff);
				if (i < 0) {
		...
		case oLDC:
			...
			break;
		case oLDV:
			reg_load_var(r0, e, (char *)(*(e->p)++), NULL);
			r_push(r0);
			break;
		case oLDS:
			x = *(e->p)++;
		...
		case oNEW:
			...
			v = alloc_var((char *)(x), NULL, ((struct class_names_s *)(*((uintptr_t *)(r0->data))))->classes[e->comm->conn]);
			if (v == NULL)
		...
		case oALI:
			x = *(e->p)++;
			r_pop(r0);
			if ((r0->attr->type & 0x0f) != MED_TYPE_STRING || r0->data == NULL) {
			...
			v = data_alias_var((char *)(x), data_find_var(e, (char *)(r0->data)));
			if (v == NULL)
				runtime("Can't alias variable");
			/* init register */
			r0->flags = 0;
			r0->object = NULL;
			r0->class = NULL;
		...
		case oVRL:
			r_pop(r0);
			v = (struct object_s *)(*((uintptr_t *)(r0->data)));
			if (r0->attr != &execute_attr_pointer || v == NULL) {
				r_push(r0); /* len aby nieco bolo */
				break;
			}
		...
		case oTOF:
			...
			if (r0->class == NULL) {
				...
				push((uintptr_t)(r0->class->classname->name));
			push(LTS);
			break;
		case oCOF:
			r_pop(r0);
		...
		case oS2C:
			...
			x = (uintptr_t)(get_class_by_name((char *)r0->data));
			if (x == 0) {
		...
		case oBIN:
			...
			*((uintptr_t *)(r0->data)) = -1;
			...
			break;
		default:
			runtime("Illegal instruction 0x%08x 0x%08x 0x%08x", cmd, (e->p)[0], (e->p)[1]);
		}
		e->cont = 0;
	}
}
...
int execute_handler(struct comm_buffer_s *comm_buff, struct event_handler_s *h, struct event_context_s *c)
{
	...
	i = execute_handler_do(&comm_buff->execute);
	if (i <= 0)
