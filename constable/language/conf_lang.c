...
{
	static struct space_s *space;
	static struct space_s *space1;
	static struct space_s *space2;
	static struct tree_s *path;
	static struct tree_s *path1;
	static struct tree_s *path2;
	static char *op_name;
	static int access_type;
	static char *tree_name;
	static int flags, ehh_list;
	static struct class_names_s *class;
	...
	case Pid2class:
		...
		x = (uintptr_t)(get_class_by_name((char *)(c->l.data)));
		if (x == 0)
	...
	case Pptnull:
		...
		break;
	case Pptpath:
		path_type = (path_type & ~LTREE_T_MASK) | LTREE_T_TREE;
		break;
	case Pptspace:
		path_type = (path_type & ~LTREE_T_MASK) | LTREE_T_SPACE;
		break;
	case Pptrecur:
		path_type |= LTREE_RECURSIVE;
		break;
	case Pptexcl:
		path_type |= LTREE_EXCLUDE;
		break;
	case Paddpath:
	...
	case Pfindspa2:
		...
		c->l.data = (uintptr_t)space_find((char *)(c->l.data));
		if (c->l.data == 0)
	...
	case Ptreeflags:
		...
		if (!strcmp("test_enter", (char *)(c->l.data)))
			...
			error("Invalid tree flag %s", (char *)(c->l.data));
		break;
	case Ptreetype:
	...
	case Pptree:
		...
		break;
	case Psgvs:
		c->l.data = (uintptr_t)(space_get_vs((struct space_s *)(c->l.data)));
		if (c->l.data == 0) {
			char **errstr = (char **) pthread_getspecific(errstr_key);
